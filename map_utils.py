...
def place_entities(room, entities, dungeon_level, colors):
    ...
    max_monsters_per_room = from_dungeon_level([[1, 1], [2, 2], [3, 4], [5, 6]], dungeon_level)
    ...
    for i in range(number_of_items):
        ...
        if not any([entity for entity in entities if entity.x == x and entity.y == y]):
            ...
            if item_chance == 'healing_potion':
                ...
                item_component = Item(use_function=cast_fireball, targeting=True, targeting_message=Message('Left-click a target, or right click to cancel.', colors.get('light_cyan')), damage=25, radius=3)
                ...
                item_component = Item(use_function=cast_confuse, targeting=True, targeting_message=Message('Left-click a target, or right click to cancel.', colors.get('light_cyan')))
 ...
 map_height, player, entities, colors):
    ...
    for r in range(max_rooms):
        ...
        for other_room in rooms:
            ...
            if num_rooms == 0:
                # if first room, place player in the center
                player.x = new_x
                player.y = new_y
            else:
                # all other rooms need to connect to previous room
...
def next_floor(player, message_log, dungeon_level, constants):
    ...
    message_log.add_message(Message('You take a short rest to recover your strength.', constants['colors'].get('light_violet)')))
